//todas as mudanças de codigo foram feitas a partir da da linha 32 até a linha 42
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
int AbreArea( int Tabuleiro[_Height_][_Width_],
     ...
     if( TabVisual[Y][X] == _FECHADA_ && Tabuleiro[Y][X] == _VAZIA_ )
     {
       ...
       //começo
                ...
                if (Y < (_Height_ - 1) && X < (_Width_ - 1)) if (Tabuleiro[Y + 1][X + 1] > 0) TabVisual[Y + 1][X + 1] = _ABERTA_; // diag inf direita
                if (Y < (_Height_ - 1) && X > 0)             if (Tabuleiro[Y + 1][X - 1] > 0) TabVisual[Y + 1][X - 1] = _ABERTA_; // diag inf esquerda
                if (Y > 0 && (X < _Width_ - 1))              if (Tabuleiro[Y - 1][X + 1] > 0) TabVisual[Y - 1][X + 1] = _ABERTA_; // diag sup direita
                if (Y > 0 && X > 0)	                         if (Tabuleiro[Y - 1][X - 1] > 0) TabVisual[Y - 1][X - 1] = _ABERTA_; // diag sup esquerda
     	//fim
		 }
